...
from services.raw_late_fusion_predictor import load_assets as load_raw_late_assets
from services.raw_late_fusion_predictor import predict as predict_raw_late_fusion
...
@st.cache_resource
def get_raw_late_fusion_assets():
    return load_raw_late_assets()
...
def find_data_file(*parts):
    candidates = [
        DATA_DIR / Path(*parts),
        APP_DIR.parent.parent / "data" / Path(*parts),
    ]
    for p in candidates:
        if p.exists():
            return p
...
@st.cache_data
def load_test_clean_for_prediction():
    path = find_data_file("raw", "test_clean.csv")
    if not path.exists():
...
@st.cache_data
def get_test_image_filenames():
    image_dir = find_data_file("raw", "images", "image_test")
    if not image_dir.exists():
        return []
    return sorted([p.name for p in image_dir.glob("*.jpg")])


def get_test_image_path(filename):
    if not filename:
        return None
    p = find_data_file("raw", "images", "image_test", filename)
    return p if p.exists() else None


def get_metadata_for_image(filename, test_df):
    if test_df.empty or not filename:
        return None

    if "image_fname" in test_df.columns:
        match = test_df[test_df["image_fname"].astype(str) == str(filename)]
        if not match.empty:
            return match.iloc[0]

    # fallback from filename pattern: image_{imageid}_product_{productid}.jpg
    m = re.match(r"image_(\d+)_product_(\d+)\.jpg", filename)
    if m and {"imageid", "productid"}.issubset(test_df.columns):
        imageid = int(m.group(1))
        productid = int(m.group(2))
        match = test_df[
            (test_df["imageid"].astype(int) == imageid) &
            (test_df["productid"].astype(int) == productid)
        ]
        if not match.empty:
            return match.iloc[0]
...
NAV_DISPLAY = {
    ...
    "10. T9ConvNeXt tool": "T9ConvNeXt tool",
}
...
# -------------------------------------------------------------------
...
elif page == "10. T9ConvNeXt tool":
    st.title("Rakuten Multimodal Prediction Tool")
    st.caption("ConvNeXt-Base image model + CamemBERT text model, late-fusion prediction.")

    st.markdown(
        """
        This tool predicts the top 3 Rakuten product categories from a product title,
        description, image, or any combination of them.
        """
    )

    try:
        assets = get_raw_late_fusion_assets()
    except Exception as e:
        st.error("Could not load prediction assets.")
        st.exception(e)
        st.stop()
    
    if st.button("Reload test_clean.csv cache"):
        load_test_clean_for_prediction.clear()
        get_test_image_filenames.clear()
        st.rerun()
    ...
    with col_left:
        ...
        # Defaults so variables exist in every branch
        designation = ""
        description = ""
        loaded_image = None
        loaded_row = None
        ...
        input_mode = st.radio(
            "Input source",
            ["Manual input", "Load from test image folder"],
            horizontal=True,
        )

        if input_mode == "Load from test image folder":
            test_df, test_clean_path = load_test_clean_for_prediction()
            image_filenames = get_test_image_filenames()

            if test_df.empty:
                st.warning(f"test_clean.csv was not found or could not be loaded at:\n\n{test_clean_path}")
            elif not image_filenames:
                st.warning("No JPG files found in raw/images/image_test.")
            else:
                selected_filename = st.selectbox(
                    "Choose image from test folder",
                    image_filenames,
                    index=0,
                )

                loaded_row = get_metadata_for_image(selected_filename, test_df)
                image_path = get_test_image_path(selected_filename)

                if image_path is not None:
                    loaded_image = Image.open(image_path).convert("RGB")

                if loaded_row is not None:
                    default_designation = safe_cell_to_text(loaded_row.get("designation", ""))
                    default_description = safe_cell_to_text(loaded_row.get("description", ""))
                else:
                    default_designation = ""
                    default_description = ""
                    st.warning("No matching row found in test_clean.csv for this image.")
            ...
            designation = st.text_input(
                "Product title / designation",
                placeholder="Example: Robot de piscine électrique",
            )
            ...
            if uploaded_image is not None:
                loaded_image = Image.open(uploaded_image).convert("RGB")

        has_text = bool((designation or "").strip() or (description or "").strip())
        has_image = loaded_image is not None

        if has_text and has_image:
            image_weight = st.slider(
                "Image weight for late fusion",
                min_value=0.0,
                max_value=1.0,
                value=0.45,
                step=0.01,
                help="Text weight is automatically set to 1 - image weight.",
            )
            st.caption(f"Image weight: {image_weight:.2f} | Text weight: {1.0 - image_weight:.2f}")
        else:
            image_weight = 0.45

        run_prediction = st.button("Predict top 3 categories", type="primary")

    with col_right:
        st.subheader("Prediction")

        # Show the selected/loaded image before prediction, but do not run the model yet.
        if not run_prediction and loaded_image is not None:
            st.image(loaded_image, caption="Selected image", width="stretch")
            st.info("Image and metadata loaded. Click Predict to run the model.")

        elif not run_prediction:
            st.info("Enter a title, description, image, or load an example from the test folder, then click Predict.")

        elif not has_text and not has_image:
            st.warning("Please provide at least one input: title, description, or image.")

        else:
            try:
                result = predict_raw_late_fusion(
                    assets=assets,
                ...
                st.markdown(f"**Prediction mode:** {result['mode']}")

                if result["mode"] == "Late fusion":
                    st.markdown(
                        f"**Fusion weights:** image `{result['image_weight']:.2f}`, "
                        f"text `{result['text_weight']:.2f}`"
                    )

                pred_df = pd.DataFrame(result["top3"])[
                    ["Rank", "prdtypecode", "Category name", "Confidence"]
                ]
                ...
                st.markdown(
                    f"""
                    <div style="border:1px solid #e5e7eb; border-radius:0.8rem; padding:1rem; background:#fafafa; margin-top:1rem;">
                        <div style="font-size:0.9rem; color:#6b7280;">Top prediction</div>
                        <div style="font-size:1.4rem; font-weight:700;">{top1['prdtypecode']} — {escape(top1['Category name'])}</div>
                        <div style="font-size:1.1rem; margin-top:0.25rem;">Confidence: <b>{top1['Confidence']}</b></div>
                    </div>
                    """,
                    unsafe_allow_html=True,
                )

                # After prediction, keep the image below the prediction results.
                if loaded_image is not None:
                    st.image(loaded_image, caption="Selected image", width="stretch")

                if result["text_used"]:
                    with st.expander("Text sent to CamemBERT"):
                        st.write(result["text_used"])
