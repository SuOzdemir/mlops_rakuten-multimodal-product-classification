...
# and CLS-feature / logit export for fusion models.
# ============================================================
...
# Note: CamemBERT computes loss internally – no external criterion.
# ------------------------------------------------------------

def run_epoch(model, loader, device, optimizer=None, scaler=None) -> tuple:
    """
    Run one full epoch (train or validation) for a HuggingFace text model.

    CamemBERT computes the cross-entropy loss internally when 'labels'
    are passed to the forward call, so no external criterion is needed.
...
def export_logits_and_features(
    ...
    """
    Extract and save CamemBERT logits and CLS-token features for fusion.

    Exports two .npy files per split:
        text_<split_name>_logits.npy              – shape (N, num_classes)
        text_<split_name>_features_<dim>d.npy     – shape (N, 768)

    The CLS token (position 0 of last_hidden_state) is used as the
    sentence-level representation for intermediate fusion.

    The loader MUST use shuffle=False to guarantee correct sample ordering.

    Args:
        model        : Trained CamemBERT model (eval mode expected).
        loader       : Ordered DataLoader (shuffle=False).
        device       : torch device.
        output_path  : Directory where .npy files are written.
        split_name   : "train" or "val" – used in filenames.
        feature_dim  : CamemBERT hidden size (768 for base model).
    """
    output_path.mkdir(parents=True, exist_ok=True)
    model.eval()

    # Robustly access the transformer backbone
    # (attribute name can differ between model versions)
    transformer_backbone = getattr(
        model, "camembert", getattr(model, "roberta", None)
    )
    if transformer_backbone is None:
        raise AttributeError(
            "Could not find transformer backbone. "
            "Expected 'camembert' or 'roberta' attribute on model."
        )

    all_logits:   list = []
    all_features: list = []

    print(f"Exporting text logits & {feature_dim}d CLS features for split: {split_name} …")
    ...
    for batch in loader:
        ...
        with autocast(device_type="cuda", enabled=torch.cuda.is_available()):
            # 1. Full model forward → logits
            outputs = model(input_ids=input_ids, attention_mask=attention_mask)
            logits  = outputs.logits

            # 2. Backbone forward → CLS token as sentence embedding
            base_outputs = transformer_backbone(
                input_ids=input_ids, attention_mask=attention_mask
            )
            features = base_outputs.last_hidden_state[:, 0, :]   # (B, 768)
    ...
    np.save(output_path / f"text_{split_name}_logits.npy",                   logits_arr)
    np.save(output_path / f"text_{split_name}_features_{feature_dim}d.npy",  features_arr)

    print(
        f"  Logits   : {logits_arr.shape}  → "
        f"{output_path / f'text_{split_name}_logits.npy'}"
    )
    print(
        f"  Features : {features_arr.shape}  → "
        f"{output_path / f'text_{split_name}_features_{feature_dim}d.npy'}"
    )
...
def evaluate_model(
    ...
    """
    Run inference, print classification report, save confusion matrix.
